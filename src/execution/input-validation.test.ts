...
describe('input validation (backward compatibility)', () => {
  ...
  it('detects type mismatches for complex types', () => {
    const schema: Record<string, InputField> = {
      items: { type: 'array', element: { type: 'string' } },
    };

    const invalidInput = { items: 'not an array' };
    const collector = validateInputData(invalidInput, schema);
    expect(collector.hasErrors()).toBe(true);
    expect(collector.getErrors()[0].message).toContain('Expected array');
  });
});
